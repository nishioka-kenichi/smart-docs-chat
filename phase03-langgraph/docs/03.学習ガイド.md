...
# 各モジュールを個別に実行
python src/agent_state.py    # State管理を学習
python src/tools.py          # ツールの動作を確認
python src/react_agent.py    # ReActパターンを理解
python src/checkpointer.py   # チェックポイント機能を確認
python src/graph_builder.py  # グラフ構造を理解
```
...
# .vscode/launch.json
{
    ...
    "configurations": [
        {
            "name": "Debug agent_state",
            "type": "python",
            "request": "launch",
            "program": "${workspaceFolder}/src/agent_state.py",
            "console": "integratedTerminal",
            "cwd": "${workspaceFolder}"
        },
        {
            "name": "Debug react_agent",
            "type": "python",
            "request": "launch",
            "program": "${workspaceFolder}/src/react_agent.py",
            "console": "integratedTerminal",
            "cwd": "${workspaceFolder}"
        }
    ]
...
# 実行して各ステップを観察
python src/main.py --verbose "テスト質問"
```
